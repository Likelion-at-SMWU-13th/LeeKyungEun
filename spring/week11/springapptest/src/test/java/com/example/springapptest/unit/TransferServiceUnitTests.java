package com.example.springapptest.unit;
...
class TransferServiceUnitTests {
     ...
     */
    @Test
    @DisplayName("정상 플로우: 계좌 이체가 성공적으로 수행된다") // 테스트 결과 리포트에 어떻게 알릴지
    ...
    void moneyTransferSenderAccountNotFound() {
        ...
        assertThrows(AccountNotFoundException.class, () -> transferService.transferMoney(999L, 2L, new BigDecimal(100)));

        // TODO: Then - changeAmount()가 호출되지 않았는지 확인 -> 계좌 못 찾았으니 금액 변경 X
        // 추가 검증
    ...
    void moneyTransferReceiverAccountNotFound() {
        ...
        // When & Then
        AccountNotFoundException exception = assertThrows(AccountNotFoundException.class, () -> transferService.transferMoney(1L, 999L, new BigDecimal(100)));
